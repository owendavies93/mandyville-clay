...
- `draft/` — Draft league state loaders and recommendation logic: `models.go` (types, re-exporting `squad`), `load.go` (DB queries), `xi.go` (thin wrappers over `squad`), `recommend.go` (candidate generation + waiver simulation), `persist.go` (recommendation logging), `draft_test.go`
