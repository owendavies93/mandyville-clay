...
```
go run ./cmd/recommendations/                      # list recent classic runs
go run ./cmd/recommendations/ -id 3                # show full details of run 3
go run ./cmd/recommendations/ -gw 2                # latest classic run for GW2
go run ./cmd/recommendations/ -game draft           # list recent draft runs
go run ./cmd/recommendations/ -game draft -id 6    # show draft run 6
```

Displays past transfer recommendations generated by `cmd/transfers`.

Flags:

| Flag | Default | Description |
|---|---|---|
| `-game` | classic | Game to show: `classic` or `draft` |
| `-season` | 2026 | Season |
| `-id` | — | Show a specific recommendation run by ID |
| `-gw` | — | Show the latest run for this gameweek |
| `-limit` | 20 | Max runs to list |
| `-league` | — | FPL draft league id (draft only; auto-detected if only one exists) |
| `-entry` | — | Override the classic entry id (`is_mine` row by default) |
| `-config`, `-db-*` | — | Database connection (same as `cmd/project`) |
